...
'''
    DenseNet的keras实现
    每层的输入为之前所有层的输出，因计算量逐层的减少及特征的复用，效率高于其他网络。
'''
...
def DenseNet121(nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.0, dropout_rate=0.0, weight_decay=1e-4, classes=1000, weights_path=None):
    ...
    global concat_axis
    if tf.keras.K.image_dim_ordering() == 'tf':
      concat_axis = 3
      img_input = tf.keras.layers.Input(shape=(224, 224, 3), name='data')
    else:
      concat_axis = 1
      img_input = tf.keras.layers.Input(shape=(3, 224, 224), name='data')
    ...
    x = tf.keras.layers.Convolution2D(nb_filter, 7, 7, subsample=(2, 2), name='conv1', bias=False)(x)
...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None, weight_decay=1e-4):
    ...
    x = tf.keras.layers.Convolution2D(inter_channel, 1, 1, name=conv_name_base+'_x1', bias=False)(x)
    ...
    x = tf.keras.layers.Convolution2D(nb_filter, 3, 3, name=conv_name_base+'_x2', bias=False)(x)
...
def transition_block(x, stage, nb_filter, compression=1.0, dropout_rate=None, weight_decay=1E-4):
    ...
    x = tf.keras.layers.Convolution2D(int(nb_filter * compression), 1, 1, name=conv_name_base, bias=False)(x)
...
def dense_block(x, stage, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1e-4, grow_nb_filters=True):
    ...
    for i in range(nb_layers):
        ...
        concat_feat = tf.keras.layers.merge([concat_feat, x], mode='concat', concat_axis=concat_axis, name='concat_'+str(stage)+'_'+str(branch))
